...
<div class="max-w-3xl mx-auto">
    <div class="mb-6">
        <h2 class="text-3xl font-bold text-gray-800">Add New Application</h2>
        <p class="text-gray-600 mt-1">Record a new job application</p>
    </div>
        ...
        <form method="POST" action="{{ route('applications.store') }}">
                ...
                <div>
                    ...
                    <select id="status" 
                        ...
                        @foreach($statuses as $status)
                            <option value="{{ $status }}" {{ old('status', 'applied') === $status ? 'selected' : '' }}>
                                {{ ucfirst($status) }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="mb-6">
                ...
                <textarea id="notes" 
                          ...
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent @error('notes') border-red-500 @enderror">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
